#!/usr/bin/env python3
"""
aggregator_with_ui.py (v70 - Kiosk Data Endpoint)

Flask app that:
 - Remembers the user's active tab after form submissions.
 - Adds a dedicated, lightweight data endpoint for kiosks.
 - Implements a highly granular, feature-by-feature permission system.
 - Admin UI dynamically hides elements based on user permissions.
 - Kiosk Control dropdown is filtered based on user's specific kiosk permissions.
 - Allows renaming and deleting of any kiosk, including the default.
 - Elevates Super User to the same permission level as System.
 - Simplifies Kiosk Control UI for image-only displays.
 - Header image now correctly replaces header text.
 - Serves a separate display.html for image-only kiosks.
...
@app.route('/kiosk_data/<kiosk_id>')
def kiosk_data(kiosk_id):
    """Endpoint to fetch only the configuration for a specific kiosk."""
    kiosk_config = get_kiosk_config(kiosk_id)
    main_config = get_full_config()
    return jsonify({
...
def admin():
    ...
    return render_template('admin.html', 
        ...
        permitted_kiosks=permitted_kiosks,
...
def edit_printer(original_name):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('admin', tab='printer-config'))
...
def manage_users():
    ...
    if action == 'add_user':
        ...
        return change_user_password(active_tab)
    elif action == 'change_role':
        return change_user_role(active_tab)
...
def change_user_password(active_tab):
    users = load_data(USERS_FILE, {})
    roles = load_data(ROLES_FILE, {})
...
def change_user_role(active_tab):
    users = load_data(USERS_FILE, {})
    roles = load_data(ROLES_FILE, {})
    logged_in_role_level = roles.get(session.get('role'), {}).get('level', 0)
    username_to_change = request.form.get('username')
...
def update_kiosk(active_tab):
    ...
    # NOTE: 'show_printers' is NOT updated here. It's handled by edit_kiosk.
...
def manage_kiosk_images():
    ...
    if image_order and image_order[0]:
        current_images_map = {img['url']: img for img in kiosk_config.get('kiosk_images', [])}
        for url in image_order:
            if url in current_images_map:
                original_index = -1
                for i, img in enumerate(kiosk_config.get('kiosk_images', [])):
                    if img['url'] == url:
                        original_index = i
                        break
                ...
                if original_index != -1:
                    try:
                        time_val = int(request.form.get(f'time_{original_index}', 5))
                    except (ValueError, TypeError):
                        time_val = 5
                    new_kiosk_images.append({'url': url, 'time': time_val})

    else:
        if 'kiosk_images' in kiosk_config and isinstance(kiosk_config['kiosk_images'], list):
            for i in range(len(kiosk_config['kiosk_images'])):
                ...
                try:
                    time_val = int(request.form.get(f'time_{i}', 5))
                except (ValueError, TypeError):
                    time_val = 5
...
def rename_kiosk(active_tab):
    old_kiosk_id = secure_filename(request.form.get('old_kiosk_id', '').strip())
    new_kiosk_id = secure_filename(request.form.get('new_kiosk_id', '').strip())
...
@app.route('/admin/edit_kiosk', methods=['POST'])
@require_permission('manage_kiosk_settings')
...
def kiosk(kiosk_id='default'):
    ...
    if not kiosk_config.get('show_printers', True):
        return render_template('display.html', kiosk_id=kiosk_id)
